#!/usr/bin/env bash
# Verify the tracked ESP32 OTA layout consumed by the portal and ArduinoOTA
# fixtures. Keep this independent of PlatformIO so CI can reject a dangerous
# CSV edit before it spends time downloading or compiling a framework.
set -euo pipefail

project_dir="$(cd "$(dirname "$0")/.." && pwd)"
table="${1:-$project_dir/partitions/esp32_ota_4m_no_fs.csv}"
